...
# === crypto/encrypt.py ===
# Шифрування хешу (у вигляді точки) за допомогою ElGamal

def encrypt_hash(hash_scalar: int, public_key: Point):
    M = hash_scalar * G
    r = secrets.randbelow(q)
    C1 = r * G
    C2 = M + r * public_key
...
# === crypto/decrypt.py ===
# Розшифрування зашифрованої точки

def decrypt_point(C1: Point, C2: Point, private_key: int) -> Point:
    S = private_key * C1
    return C2 - S

# Перевірка відповідності точки з хешем

def confirm_point(M_decrypted: Point, hash_scalar: int) -> bool:
    expected = hash_scalar * G
